...
export VLN_ANNOTATION="${VLN_ANNOTATION:-/mnt/data/vmo-ai-task/anhdh35/JanusVLN/train_r2r_rxr_15deg.json}"
...
if [[ "$VLN_TRAIN_MODE" == "finetune" ]]; then
    ...
    OUTPUT_DIR="${OUTPUT_DIR:-/mnt/data/vmo-ai-task/dungpq6/model-checkpoint/Qwen-base_r2r_rxr_new_data}"
fi
...
GEOMETRY_ENCODER_STREAMING="${GEOMETRY_ENCODER_STREAMING:-false}"
FEATURE_FUSION_METHOD="${FEATURE_FUSION_METHOD:-deepstack_language_add}"
GEOMETRY_FUSION_LAYERS="${GEOMETRY_FUSION_LAYERS:-0 0 0}"
GEOMETRY_ENCODER_LAYERS="${GEOMETRY_ENCODER_LAYERS:-0 0 0}"
REFERENCE_FRAME="${REFERENCE_FRAME:-first}"
GEOMETRY_FUSION_SCALE="${GEOMETRY_FUSION_SCALE:-0}"   # JanusVLN-style lam on geometry delta (saved to config)
GEOMETRY_FRAME_STRICT="${GEOMETRY_FRAME_STRICT:-false}" # Step 1: per-frame geometry vs last-frame broadcast (default off = baseline)
