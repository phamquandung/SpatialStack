...
class LazySupervisedDataset(Dataset):
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in sources[0]:
            ...
            if isinstance(image_file, List):
                ...
                for file in image_file:
                    ret = prepare_image_inputs(
                        ...
                        geometry_encoder_streaming=getattr(
                            self.data_args, "geometry_encoder_streaming", False
                        ),
                    )
            ...
            if vln_debug.is_enabled():
                vln_debug.log_dataset_sample(
                    sample_idx=i,
                    sample_id=self.list_data_dict[i].get("id", i),
                    raw_images=image_file,
                    grid_thw=grid_thw,
...
class DataCollatorForSupervisedDataset(object):
    ...
    tokenizer: transformers.PreTrainedTokenizer
    data_args: Optional[object] = None
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if "geometry_encoder_inputs" in instances[0]:
            def stack_geometry_frames(frame_tensors):
                max_h = max(t.shape[-2] for t in frame_tensors)
                max_w = max(t.shape[-1] for t in frame_tensors)
                padded = []
                for tensor in frame_tensors:
                    ...
                    if pad_h or pad_w:
                        tensor = torch.nn.functional.pad(
                            tensor, (0, pad_w, 0, pad_h), mode="constant", value=1.0
                        )
                    padded.append(tensor)
                return torch.stack(padded)

            geometry_encoder_inputs = [
                stack_geometry_frames(instance["geometry_encoder_inputs"])
                for instance in instances
            ]
            ...
            batch["tag"] = tags[0]
            if vln_debug.is_enabled() and grid_thw is not None:
                merge_size = getattr(
                    getattr(self.data_args, "image_processor", None), "merge_size", 2
                )
                vln_debug.log_collator_batch(
                    geometry_encoder_inputs=geometry_encoder_inputs,
