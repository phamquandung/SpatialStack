...
from .feature_fusion import FeatureFusionModule, FeatureFusionConfig, GeometryFeatureMerger, MultiLayerFeatureFusionConfig, MultiLayerFeatureFusionModule
...
class Qwen2_5_VisionTransformerPretrainedModel(Qwen2_5_VLPreTrainedModel):
    ...
    def forward(
        ...
        geometry_layer_features: Optional[Dict[int, Union[torch.Tensor, List[torch.Tensor]]]] = None,
        fusion_module: Optional[nn.Module] = None,
        include_camera_token: Optional[bool] = False,
        pos_encoding_type: Optional[str] = "none",
        capture_layer_indices: Optional[List[int]] = None,
        ...
        Args:
            ...
            geometry_layer_features (`Dict[int, Union[torch.Tensor, List[torch.Tensor]]]`, *optional*):
                Mapping from fusion block indices to concatenated geometry feature tensors. Each tensor (or list of tensors
                when multiple features fuse into the same block) stores the per-image patch tokens (and optional camera
                tokens) aligned with the ordering expected by the fusion modules.
            capture_layer_indices (`List[int]`, *optional*):
                Vision block indices whose (pre-merger) hidden states should be returned for downstream fusion.
            return_layer_features (`bool`, *optional*, defaults to `False`):
                Whether to return the captured layer features alongside the merged vision embeddings.
            
        Returns:
            `torch.Tensor` with the final hidden states.
        """
        capture_layer_indices = capture_layer_indices or []
        capture_layer_indices_set = set(capture_layer_indices)
        captured_layer_states: Dict[int, torch.Tensor] = {}
        ...
        # Pre-compute 2D sin/cos absolute positional embeddings for fusion (once per forward)
        vis_pos_embed_per_image = None
        geo_pos_embed_per_image = None
        if (
            geometry_layer_features is not None
            and fusion_module is not None
            and pos_encoding_type is not None
            and pos_encoding_type != "none"
            and hasattr(fusion_module, "vis_hidden_size")
        ):
            # Determine grid size and number of images
            num_grid_h, num_grid_w = grid_thw[0][1:].tolist()
            # Infer number of images from the first available geometry layer
            if len(geometry_layer_features) > 0:
                first_key = next(iter(geometry_layer_features.keys()))
                num_images = geometry_layer_features[first_key].shape[0]
            else:
                num_images = 0

            if num_images > 0:
                # Build base grid indices [H, W, 2]
                row_idx = torch.arange(num_grid_h, device=hidden_states.device)
                col_idx = torch.arange(num_grid_w, device=hidden_states.device)
                grid_y, grid_x = torch.meshgrid(row_idx, col_idx, indexing="ij")
                patch_pos = torch.stack([grid_y, grid_x], dim=-1)  # [H, W, 2]
                patch_pos = patch_pos.unsqueeze(0).repeat(num_images, 1, 1, 1)  # [B, H, W, 2]

                # Rearrange to match spatial merge + window order
                patch_pos = patch_pos.reshape(
                    num_images,
                    num_grid_h // self.spatial_merge_size,
                    self.spatial_merge_size,
                    num_grid_w // self.spatial_merge_size,
                    self.spatial_merge_size,
                    -1,
                )
                patch_pos = patch_pos.permute(0, 1, 3, 2, 4, 5)
                patch_pos = patch_pos.reshape(seq_len // self.spatial_merge_unit, self.spatial_merge_unit, -1)
                patch_pos = patch_pos[window_index, :, :]
                patch_pos = patch_pos.reshape(num_images, seq_len // num_images, -1)  # [B, N, 2]
                patch_pos = patch_pos + 1  # shift by 1; reserve (0,0) for camera token if present

                # Create full grid of embeddings once: size (H+1, W+1, C)
                from .position_utils import get_2d_sincos_pos_embed

                vis_hidden_size = getattr(fusion_module, "vis_hidden_size")
                pos_grid = get_2d_sincos_pos_embed(
                    num_grid_h + 1, num_grid_w + 1, vis_hidden_size, hidden_states.device
                ).to(hidden_states.dtype)
                pos_grid = pos_grid.reshape(num_grid_h + 1, num_grid_w + 1, -1)

                # Index for vision tokens
                ...
                vis_pos_embed_per_image = pos_grid[h_idx_vis, w_idx_vis]  # [B, N, C]

                # Geometry tokens: optionally prepend camera token at (0,0)
        ...
        for layer_num, blk in enumerate(self.blocks):
            ...
            # deep stack
            if (geometry_layer_features is not None) and (layer_num in geometry_layer_features) and (fusion_module is not None):
                # align geo token and vsi token (support multiple geometry features per layer)
                geo_features_for_layer = geometry_layer_features[layer_num]
                if isinstance(geo_features_for_layer, torch.Tensor):
                    geo_features_for_layer = [geo_features_for_layer]

                fusion_layers = fusion_module.get_fusion_layer(layer_num)
                shared_fusion = len(fusion_layers) == 1
                if not shared_fusion and len(fusion_layers) != len(geo_features_for_layer):
                    raise ValueError(
                        f"Mismatched geometry features ({len(geo_features_for_layer)}) and fusion blocks "
                        f"({len(fusion_layers)}) for layer {layer_num}."
                    )

                for fusion_idx, geo_layer_feature in enumerate(geo_features_for_layer):
                    num_images, _, _ = geo_layer_feature.shape
                    # reshape hidden_states
                    hidden_states = hidden_states.reshape(num_images, seq_len // num_images, -1)
                    # reshape patch tokens
                    if not include_camera_token: 
                        camera_token = None
                        patch_tokens = geo_layer_feature
                    else:
                        camera_token = geo_layer_feature[:, 0:1]
                        patch_tokens = geo_layer_feature[:, 1:]
                    patch_tokens = patch_tokens.reshape(seq_len // self.spatial_merge_unit, self.spatial_merge_unit, -1)
                    patch_tokens = patch_tokens[window_index, :, :]
                    patch_tokens = patch_tokens.reshape(num_images, seq_len // num_images, -1)

                    if not include_camera_token: 
                        geo_layer_feature = patch_tokens
                    else:
                        geo_layer_feature = torch.cat([camera_token, patch_tokens], dim=1)
                    # fetch precomputed embeddings if available
                    vis_pos_embed = vis_pos_embed_per_image
                    geo_pos_embed = geo_pos_embed_per_image

                    hidden_states = fusion_module(
                        hidden_states,
                        geo_layer_feature,
                        layer_num,
                        vis_pos_embed,
                        geo_pos_embed,
                        fusion_layer_idx=fusion_idx if not shared_fusion else 0,
                    )
                    # reshape hidden_states
                    hidden_states = hidden_states.reshape(seq_len, -1)

            # collect pre-merger hidden states when requested
            if layer_num in capture_layer_indices_set:
                captured_layer_states[layer_num] = hidden_states

        hidden_states = self.merger(hidden_states)
        reverse_indices = torch.argsort(window_index)
        hidden_states = hidden_states[reverse_indices, :]

        if not return_layer_features or len(captured_layer_states) == 0:
            return hidden_states

        # Reorder captured states to original patch order and reshape per image
        num_images = grid_thw.shape[0]
        num_groups = seq_len // self.spatial_merge_unit
        layer_features: Dict[int, torch.Tensor] = {}
        for layer_idx, layer_states in captured_layer_states.items():
            layer_states = layer_states.reshape(num_groups, self.spatial_merge_unit, -1)
            layer_states = layer_states[reverse_indices, :, :]
            layer_states = layer_states.reshape(seq_len, -1)
            layer_states = layer_states.reshape(num_images, seq_len // num_images, -1)
            layer_features[layer_idx] = layer_states
...
class Qwen2_5_VLModel(Qwen2_5_VLPreTrainedModel):
    ...
    def forward(
        ...
        cache_position: Optional[torch.LongTensor] = None,
        geometry_layer_features: Optional[List[torch.Tensor]] = None,
        fusion_module: Optional[nn.Module] = None,
        ...
        # Pre-compute positional embeddings for geometry fusion when applicable
        ...
        if (
            ...
            and len(geometry_layer_features) > 0
        ):
            from .position_utils import get_2d_sincos_pos_embed
            ...
            if num_images > 0:
                num_grid_h, num_grid_w = grid_thw[0][1:].tolist()
                num_merged_grid_h = num_grid_h // fusion_module.config.spatial_merge_size
                num_merged_grid_w = num_grid_w // fusion_module.config.spatial_merge_size
                lang_hidden_size = getattr(fusion_module, "lang_hidden_size")

                pos_grid = get_2d_sincos_pos_embed(
                    num_merged_grid_h + 1, num_merged_grid_w + 1, lang_hidden_size, hidden_states.device
                ).to(hidden_states.dtype)
                ...
                patch_pos = patch_pos + 1  # reserve (0, 0) for camera token if present
        ...
        for layer_idx, decoder_layer in enumerate(self.layers):
            ...
            if geometry_layer_features is not None and layer_idx in geometry_layer_features:
                ...
                fused = fusion_module(vision_tokens, geometry_layer_features[layer_idx], layer_idx, vis_pos_embed_per_image, geo_pos_embed_per_image)
...
class Qwen2_5_VLForConditionalGenerationWithVGGT(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        # Create language-side fusion module (can reuse geometry fusion or build from vision features)
        fusion_method = getattr(config, "feature_fusion_method", "add")
        geometry_fusion_layers = getattr(config, "geometry_fusion_layers", None)
        vision_language_fusion_layers = getattr(config, "vision_language_fusion_layers", None)
        use_vision_language_fusion = vision_language_fusion_layers is not None
        if "deepstack_language" in fusion_method and geometry_fusion_layers is not None:
            if use_vision_language_fusion:
                fusion_config = MultiLayerFeatureFusionConfig(
                    ...
                    geo_hidden_size=self.visual.config.hidden_size,
                ...
                # fall back to geometry fusion module when available
    ...
    def _init_geometry_encoder(self, config):
        ...
        if "deepstack" in getattr(config, "feature_fusion_method", "add"):
            # Create multi layer future fusion module
            multi_layer_fusion_config = MultiLayerFeatureFusionConfig(
                fusion_method=getattr(config, "feature_fusion_method", "add"),
                vis_hidden_size=self.visual.config.hidden_size,
                geo_hidden_size=self.geometry_encoder.get_feature_dim(),
                lang_hidden_size=config.hidden_size,
                geometry_fusion_layers=getattr(config, "geometry_fusion_layers", None),
                pos_encoding_type=getattr(config, "pos_encoding_type", "none"),
                spatial_merge_size=config.vision_config.spatial_merge_size,
            ...
            self.multi_layer_feature_fusion = MultiLayerFeatureFusionModule(multi_layer_fusion_config)
        else:
            # Create geometry merger list
            ...
            if geometry_encoder_layers is not None and len(geometry_encoder_layers) > 1:
                self.geometry_merger_list = nn.ModuleList(
                    ...
                    ) for _ in geometry_encoder_layers)
            else:
    ...
    def _collect_geometry_layer_features(self, geometry_encoder_inputs):
        """Collect geometry encoder outputs for deepstack vision fusion."""
        ...
        assert len(geometry_encoder_inputs) == 1, "just input 1 batch"
    ...
    def _process_geometry_features(self, image_embeds, geometry_encoder_inputs, geometry_encoder_layers=[-2]):
        ...
        for bn in range(batch_size):
            if geometry_encoder_inputs[bn].shape[0] > 0:
                ...
                features = self.geometry_encoder.encode_layers(geometry_encoder_inputs[bn], layer_indices=geometry_encoder_layers)[-1].to(image_embeds.dtype)
    ...
    def _process_multi_geometry_features(self, image_embeds, geometry_encoder_inputs, geometry_encoder_layers=[-2]):
        geo_feature_list = self.geometry_encoder.encode_layers(geometry_encoder_inputs[0], layer_indices=geometry_encoder_layers)
        ...
        for geo_feature, geo_merger in zip(geo_feature_list, self.geometry_merger_list):
            geo_feature = geo_feature.reshape(n_image, height // self.geometry_encoder.patch_size, width // self.geometry_encoder.patch_size, -1)
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Ensure image_mask is always defined across all code paths, including
        # generation steps where pixel_values are dropped after the first step.
        image_mask: Optional[torch.Tensor] = None
        vision_layer_features: Optional[Dict[int, torch.Tensor]] = None
        ...
        should_capture_vision_layers = (
            ...
            and "deepstack_language" in getattr(self.config, "feature_fusion_method", "add")
        )
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None:
                ...
                return_layer_features = should_capture_vision_layers
                visual_common_kwargs = dict(
                    capture_layer_indices=vision_language_fusion_layers if should_capture_vision_layers else None,
                    return_layer_features=return_layer_features,
                )
                if "deepstack_vision" in getattr(self.config, "feature_fusion_method", "add"):
                    # encode geo feature
                    ...
                    visual_outputs = self.visual(
                        ...
                        **visual_common_kwargs,
                        )
                    image_embeds, vision_layer_features = (
                        visual_outputs if return_layer_features else (visual_outputs, None)
                    )
                elif "deepstack_language" in getattr(self.config, "feature_fusion_method", "add"):
                    visual_outputs = self.visual(pixel_values, grid_thw=image_grid_thw, **visual_common_kwargs)
                    image_embeds, vision_layer_features = (
                        visual_outputs if return_layer_features else (visual_outputs, None)
                    )
                else:
                    ...
                    if getattr(self.config, 'use_geometry_encoder', False) and geometry_encoder_inputs is not None:
                        geometry_encoder_layers = getattr(self.config, "geometry_encoder_layers", None)
                        if not geometry_encoder_layers:
                            geometry_encoder_layers = [-2]
                        if len(geometry_encoder_layers) == 1:
                            image_embeds = self._process_geometry_features(image_embeds, geometry_encoder_inputs, geometry_encoder_layers=geometry_encoder_layers)
                        else:
                            image_embeds = self._process_multi_geometry_features(image_embeds, geometry_encoder_inputs, geometry_encoder_layers=geometry_encoder_layers)
        ...
        fusion_module = None
        grid_thw = None
        include_camera_token = None
        # Only apply deepstack_language fusion on the first decode step (when cache starts at 0).
        # During subsequent generation steps, we skip fusion to avoid mismatched masks/tokens.
        if (
            "deepstack_language" in getattr(self.config, "feature_fusion_method", "add")
            and (cache_position is None or (isinstance(cache_position, torch.Tensor) and cache_position[0] == 0))
        ):
            geometry_fusion_layers = getattr(self.config, "geometry_fusion_layers", None)
            fusion_module = self.language_feature_fusion or self.multi_layer_feature_fusion
            grid_thw = image_grid_thw
            include_camera_token = getattr(self.config, "include_camera_token", False)

            if vision_layer_features is not None:
                if geometry_fusion_layers is None or vision_language_fusion_layers is None:
                    raise ValueError("geometry_fusion_layers and vision_language_fusion_layers must be set for vision deepstack language fusion.")
                if len(geometry_fusion_layers) != len(vision_language_fusion_layers):
                    raise ValueError(
                ...
                for decoder_layer, vision_layer in zip(geometry_fusion_layers, vision_language_fusion_layers):
                    if vision_layer not in vision_layer_features:
                        continue
                    geometry_layer_features.setdefault(decoder_layer, []).append(vision_layer_features[vision_layer])
            else:
                # encode geo feature
                geometry_layer_features = self._collect_geometry_layer_features(geometry_encoder_inputs)
            if geometry_layer_features is not None and fusion_module is None:
                fusion_module = self.language_feature_fusion or self.multi_layer_feature_fusion
