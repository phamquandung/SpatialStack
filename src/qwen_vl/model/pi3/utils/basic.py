...
def load_images_as_tensor_flexible(path='data/truck', interval=1, resize_mode='auto', target_width=None, target_height=None, PIXEL_LIMIT=255000):
    """
    Loads images from a directory or video with flexible resizing options.
    All output dimensions will be multiples of 14 for model compatibility.
    
    Args:
        path (str): Path to image directory or video file
        interval (int): Sampling interval for images/frames
        resize_mode (str): Resize mode - 'auto', 'fixed', 'width', 'height'
            - 'auto': Automatic sizing based on PIXEL_LIMIT (original behavior)
            - 'fixed': Resize to target dimensions (rounded down to nearest multiple of 14)
            - 'width': Scale proportionally based on target_width (adjusted to multiple of 14)
            - 'height': Scale proportionally based on target_height (adjusted to multiple of 14)
        target_width (int): Target width for resizing
        target_height (int): Target height for resizing
        PIXEL_LIMIT (int): Maximum pixels when using 'auto' mode
    
    Returns:
        torch.Tensor: Stacked tensor of shape [N, 3, H, W] where H and W are multiples of 14
    
    Examples:
        # Auto mode (original behavior)
        imgs = load_images_as_tensor_flexible('path/to/images', resize_mode='auto')
        
        # Fixed size (will be adjusted to nearest multiple of 14)
        imgs = load_images_as_tensor_flexible('path/to/images', resize_mode='fixed', 
                                            target_width=512, target_height=384)  # -> 504x378
        
        # Proportional scaling by width (width adjusted to multiple of 14)
        imgs = load_images_as_tensor_flexible('path/to/images', resize_mode='width', 
                                            target_width=1024)  # -> 1022x? (both multiples of 14)
        
        # Proportional scaling by height (height adjusted to multiple of 14)
        imgs = load_images_as_tensor_flexible('path/to/images', resize_mode='height', 
                                            target_height=768)  # -> ?x756 (both multiples of 14)
    """
    sources = [] 
    
    # --- 1. Load image paths or video frames ---
    if isinstance(path, list):
        sources = [Image.open(p).convert('RGB') for p in path if osp.isfile(p)]
    elif osp.isdir(path):
        print(f"Loading images from directory: {path}")
        filenames = sorted([x for x in os.listdir(path) if x.lower().endswith(('.png', '.jpg', '.jpeg'))])
        for i in range(0, len(filenames), interval):
    ...
    # --- 2. Determine target size based on resize mode ---
    first_img = sources[0]
    W_orig, H_orig = first_img.size
    
    if resize_mode == 'auto':
        # Original automatic sizing logic
        scale = math.sqrt(PIXEL_LIMIT / (W_orig * H_orig)) if W_orig * H_orig > 0 else 1
        W_target, H_target = W_orig * scale, H_orig * scale
        k, m = round(W_target / 14), round(H_target / 14)
        while (k * 14) * (m * 14) > PIXEL_LIMIT:
        ...
        # Fixed size - must specify both width and height
        if target_width is None or target_height is None:
            raise ValueError("Both target_width and target_height must be specified for 'fixed' mode")
        # Ensure dimensions are multiples of 14
        TARGET_W = max(14, (target_width // 14) * 14)
        TARGET_H = max(14, (target_height // 14) * 14)
        
    elif resize_mode == 'width':
        # Proportional scaling based on width
        if target_width is None:
            raise ValueError("target_width must be specified for 'width' mode")
        # Ensure target_width is multiple of 14
        TARGET_W = max(14, (target_width // 14) * 14)
        scale = TARGET_W / W_orig
        TARGET_H = max(14, (int(H_orig * scale) // 14) * 14)
        
    elif resize_mode == 'height':
        # Proportional scaling based on height
        if target_height is None:
            raise ValueError("target_height must be specified for 'height' mode")
        # Ensure target_height is multiple of 14
        TARGET_H = max(14, (target_height // 14) * 14)
        scale = TARGET_H / H_orig
        TARGET_W = max(14, (int(W_orig * scale) // 14) * 14)
        
    else:
        raise ValueError(f"Unsupported resize_mode: {resize_mode}. Must be 'auto', 'fixed', 'width', or 'height'")
    
    # --- 3. Resize images and convert them to tensors ---
    tensor_list = []
    to_tensor_transform = transforms.ToTensor()
